...
- ❌ **Sem decomposição de prosa em blocos** — parágrafos, headings e callouts ficam como sintaxe no content (§2.1).
...
### 2.1 A decisão central: onde a fronteira do objeto começa
...
> **No MVP**, `Structure` de uma `Note` tem apenas um nível: os artefatos tipados embutidos nela (§2.1). Não há aninhamento — artefatos são folhas.
...
> **Granularidade não resolve isso.** Decompor o conteúdo em `Paragraph` não preserva a posição de um link: um parágrafo com dois links continua sendo um único bloco de texto com dois links no meio dele. Âncoras são necessárias em **qualquer** nível de granularidade — e, com elas, a prosa mantida no content não perde nada de posição. O que a granularidade compra é outra coisa: endereçar um trecho de prosa, transcluí-lo e dar-lhe metadados próprios.
...
O formato `ckm/text` é, portanto, uma **string legível** — sintaxe de Markdown mais âncoras. A sintaxe cobre a prosa e sua formatação: parágrafos, headings, listas, callouts, ênfase, código inline. Tabelas e blocos de código **não** estão aqui: são artefatos tipados, e vivem como objetos embutidos por âncora (§2.1).
...
| Analyzer | Registra-se para | Fase | O que extrai |
| --- | --- | --- | --- |
| **Markdown Analyzer** | `Note` | 2 | Tags, títulos, estrutura de headings |
...
**A nota é a unidade de conflito**, mesmo tendo filhos. Seus artefatos embutidos não têm versão independente: são escritos dentro da mesma transação, condicionada à `version` do pai. Duas escritas concorrentes na mesma nota colidem no item pai, e nenhuma consegue alterar filhos sem passar por ele.

Isso funciona porque a árvore tem um nível e os filhos são folhas sem existência autônoma (§2.1). Consistência em nível de árvore só vira problema real na Fase 4, com aninhamento.
