...
**A decisão central** não é traçada por tamanho, e sim por **opacidade**:

> **É objeto quando o corpo está em outra linguagem — opaco à prosa e ao mecanismo de âncora.**

Parágrafos, headings, listas, callouts e **tabelas** são prosa: suas células e frases contêm links, que precisam ser âncoras como qualquer outro ponto do texto. Ficam como sintaxe no conteúdo.

Um bloco de código não: `[[algo]]` dentro de um exemplo de SQL **não é um link**, e tratá-lo como tal seria um bug. Ser objeto separado torna o `CodeBlock` naturalmente imune ao parser de âncoras — e essa imunidade é a razão da exceção, não o tamanho ou a importância do bloco.

O que fica de fora é decompor a **prosa** em objetos, que é a parte cara: parágrafos têm delimitação ambígua e aparecem às dezenas por nota. Isso preserva integralmente o modelo canônico — descer a granularidade depois é **adicionar mais tipos de filho**, não redesenhar o modelo.

### Dentro do MVP

- ✅ `Note`, `Collection` e `CodeBlock` como Knowledge Objects
- ✅ Diagramas **Mermaid** como `CodeBlock` — consultáveis no grafo por tipo e linguagem
- ✅ API REST completa — objetos, grafo, backlinks, coleções e importação
...
- ❌ Decomposição da prosa — parágrafos, headings, callouts, listas e tabelas viajam como sintaxe *(Fase 4)*
...
O catálogo completo de tipos previstos inclui `Collection`, `Note`, `CodeBlock`, `Heading`, `Paragraph`, `Callout`, `List`, `Table`, `Image`, `Attachment` e `Quote`. **O MVP implementa `Collection`, `Note` e `CodeBlock`** — os de prosa existem como sintaxe dentro do conteúdo, preservados no round-trip e renderizados, mas ainda não endereçáveis como objetos.
...
| Fase | Escopo |
| --- | --- |
| **1 — MVP** | `Note`, `Collection`, `CodeBlock`, relations de 1ª classe, API REST, renderers Markdown + JSON, eventos |
